...
const DEFAULT_PRODUCT_IMAGE = "data/image-backup/0001-p175-70r12-joyroad-hp-rx3-80t.jpg";
...
const initSheetSyncButton = () => {
  const syncButton = document.getElementById("syncSheetBtn");
  const statusNode = document.getElementById("syncSheetStatus");
  ...
  syncButton.addEventListener("click", async () => {
    ...
    try {
      const response = await fetch(`${STRIPE_API_BASE}/api/products/sync`, {
        method: "POST",
        headers: {
      ...
      setStatus(`Inventario actualizado (${Number(data?.count || 0)} productos).`);
      document.dispatchEvent(new CustomEvent("products:synced"));
    } catch (error) {
      setStatus(String(error?.message || "Error al sincronizar inventario."), true);
    } finally {
      syncButton.disabled = false;
      syncButton.textContent = "Actualizar desde Google Sheet";
    }
  });
};
...
const initCatalogFilters = () => {
  ...
  document.addEventListener("products:synced", loadCatalogFromApi);
};
  ...
  try {
    ...
    const validImageProducts = products
      ...
      .filter((item) => {
        ...
        return Boolean(image) && !normalized.includes("/logos/") && !normalized.includes("logotusllantas");
      });
    ...
    checkoutButton.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!response.ok || !data.url) {
          throw new Error(
            data.error ||
              "No se pudo iniciar el pago. Verifica que el backend de Stripe este corriendo."
          );
        }
...
const initRegisterForm = () => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (connectionError) {
        setFeedback("No se pudo conectar al servidor. Verifica que el backend este activo.", true);
      } else {
